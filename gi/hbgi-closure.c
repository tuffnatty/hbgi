 ...
 *
 * Most of the logic in this file is based on pygi-closure.c from pygobject
 * library:
 *
...
static void
_hbgi_closure_assign_hbitem_to_out_argument(gpointer out_arg, PHB_ITEM item,
                                            GITypeInfo *type_info,
                                            GITransfer transfer)
{
    GIArgument arg = _hbgi_argument_from_object(item, type_info, transfer);
...
static GIArgument *
_hbgi_closure_convert_ffi_arguments (GICallableInfo *callable_info, void **args)
{
    gint num_args, i;
    GIArgInfo *arg_info;
    GITypeInfo *arg_type;
...
static gboolean
_hbgi_closure_convert_arguments (GICallableInfo *callable_info, void **args,
                                 void *user_data, PHB_ITEM *hb_args,
                                 GIArgument **out_args)
{
    ...
    *hb_args = NULL;
    *hb_args = hb_itemArrayNew(n_args);
    if (*hb_args == NULL)
        goto error;

    *out_args = NULL;
    *out_args = g_new0 (GIArgument, n_args);
    g_args = _hbgi_closure_convert_ffi_arguments (callable_info, args);
    ...
    for (i = 0; i < n_args; i++) {
        ...
        if (direction == GI_DIRECTION_IN || direction == GI_DIRECTION_INOUT) {
            ...
            if (direction == GI_DIRECTION_IN && arg_tag == GI_TYPE_TAG_VOID &&
                ...
                if (user_data == NULL) {
                    value = hb_itemNew(NULL);
                } else {
                    value = hb_itemNew(user_data);
                }
            } else {
                ...
                value = _hbgi_argument_to_object (arg, arg_type, transfer);
                if (value == NULL) {
            ...
            hb_arraySet(*hb_args, n_in_args + 1, value);
            n_in_args++;
...
static void
_hbgi_closure_set_out_arguments (GICallableInfo *callable_info,
                                 PHB_ITEM hb_retval, GIArgument *out_args,
                                 void *resp)
{
    int n_args, i, i_hb_retval, i_out_args;
    ...
    if (return_type_tag != GI_TYPE_TAG_VOID) {
        ...
        if (HB_IS_ARRAY(hb_retval)) {
            PHB_ITEM item = hb_arrayGetItemPtr(hb_retval, 1);
            _hbgi_closure_assign_hbitem_to_out_argument (resp, item,
                return_type_info, transfer);
        } else {
            _hbgi_closure_assign_hbitem_to_out_argument (resp, hb_retval,
                return_type_info, transfer);
        }
        i_hb_retval++;
    }
    ...
    for (i = 1; i < n_args; i++) {
        ...
        if (direction == GI_DIRECTION_OUT || direction == GI_DIRECTION_INOUT) {
            ...
            if (HB_IS_ARRAY(hb_retval)) {
                PHB_ITEM item = hb_arrayGetItemPtr(hb_retval, i_hb_retval + 1);
                _hbgi_closure_assign_hbitem_to_out_argument (
                    out_args[i_out_args].v_pointer, item, type_info, transfer);
            } else if (i_hb_retval == 0) {
                _hbgi_closure_assign_hbitem_to_out_argument (
                    out_args[i_out_args].v_pointer, hb_retval, type_info,
                    transfer);
            } else
...
void
_hbgi_closure_handle (ffi_cif *cif,
                      void    *result,
                      void   **args,
                      void    *data)
{
    HbGICClosure *closure = data;
    ...
    if (!_hbgi_closure_convert_arguments ( (GICallableInfo *) closure->info, args,
                                           closure->user_data,
                                           &hb_args, &out_args)) {
        /*if (PyErr_Occurred ())
            PyErr_Print();*/
    ...
    for (i = 1; i <= hb_arrayLen(hb_args); i++) {
        hb_vmPush(hb_arrayGetItemPtr(hb_args, i));
    }
    if (HB_IS_SYMBOL(closure->function)) {
        hb_vmProc(hb_arrayLen(hb_args));
    } else {
        hb_vmEval(hb_arrayLen(hb_args));
    }
    retval = hb_stackReturnItem();
    hb_itemRelease(hb_args);

    _hbgi_closure_set_out_arguments (closure->info, retval, out_args, result);
    ...
    */
    switch (closure->scope) {
        ...
        case GI_SCOPE_TYPE_ASYNC:
            /* Append this HbGICClosure to a list of closure that we will free
...
void _hbgi_invoke_closure_free (gpointer data)
{
    HbGICClosure* invoke_closure = (HbGICClosure *) data;

    hb_itemRelease(invoke_closure->function);
    ...
    hb_itemRelease(invoke_closure->user_data);

    g_slice_free (HbGICClosure, invoke_closure);
}


HbGICClosure*
_hbgi_make_native_closure (GICallableInfo* info,
                           GIScopeType scope,
                           PHB_ITEM hb_function,
                           gpointer hb_user_data)
{
    HbGICClosure *closure;
    ffi_closure *fficlosure;

    /* Begin by cleaning up old async functions */
    g_slist_foreach (async_free_list, (GFunc) _hbgi_invoke_closure_free, NULL);
    g_slist_free (async_free_list);
    async_free_list = NULL;

    /* Build the closure itself */
    closure = g_slice_new0 (HbGICClosure);
    closure->info = (GICallableInfo *) g_base_info_ref ( (GIBaseInfo *) info);
    closure->function = hb_itemNew(hb_function);
    closure->user_data = hb_itemNew(hb_user_data);

    fficlosure =
        g_callable_info_prepare_closure (info, &closure->cif, _hbgi_closure_handle,
                                         closure);
